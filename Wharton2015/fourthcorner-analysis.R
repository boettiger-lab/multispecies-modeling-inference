rm(list = ls())
library(ade4)
source("fourthcorner-auxilaryfunctions.R") ## R script containing functions for fitting models for fourth-corner analysis
...
##########	
basic.nolvmodfile <- make.boralmodel.fourthcorner(family = "binomial", X.eff = "ran", site.eff = "ran", num.lv = 0, fc.eff = T, filename = "nolvint.txt")
jags.data <- list(y = aravo.pa, n = nrow(aravo.pa), p = ncol(aravo.pa), traits = as.vector(scale(aravo$traits$SLA[sel.spp])), enviro = as.vector(scale(aravo$env$Snow)))
fit.nolvint <- jags(data = jags.data, inits = gen.inits, parameters.to.save = c("mean.X.eff", "X.params", "quad.X.params", "sigma2.X.eff", "sigma2.site", "all.params","inter.params","site.params"), model.file = "nolvint.txt", n.chains = 1, n.iter = 55000, n.burnin = 5000, n.thin = 50)
...
##########
basic.modfile <- make.boralmodel.fourthcorner(family = "binomial", X.eff = "ran", site.eff = "ran", num.lv = 2, fc.eff = T, filename = "lvint.txt")
jags.data <- list(y = aravo.pa, n = nrow(aravo.pa), p = ncol(aravo.pa), traits = as.vector(scale(aravo$traits$SLA[sel.spp])), enviro = as.vector(scale(aravo$env$Snow)), num.lv = 2)
fit.lvint <- jags(data = jags.data, inits = gen.inits, parameters.to.save = c("mean.X.eff","quad.X.params","X.params", "sigma2.X.eff", "sigma2.site","all.params","inter.params","lvs"), model.file = "lvint.txt", n.chains = 1, n.iter = 55000, n.burnin = 5000, n.thin = 50)
...
##########
basic.modfile <- make.boralmodel.fourthcorner(family = "binomial", X.eff = "none", site.eff = "ran", num.lv = 2, fc.eff = F, filename = "purelv.txt")
jags.data <- list(y = aravo.pa, n = nrow(aravo.pa), p = ncol(aravo.pa), num.lv = 2)
fit.purelv2 <- jags(data = jags.data, inits = gen.inits, parameters.to.save = c("site.params","sigma2.site", "all.params","lvs"), model.file = "purelv.txt", n.chains = 1, n.iter = 55000, n.burnin = 5000, n.thin = 50)
...
purelv2.rescov <- (fit.purelv2$lv.coefs.mean[,2:3]%*%t(fit.purelv2$lv.coefs.mean[,2:3]))
lv2.rescov <- (1*fit.lvint$lv.coefs.mean[,2:3]%*%t(1*fit.lvint$lv.coefs.mean[,2:3]))
