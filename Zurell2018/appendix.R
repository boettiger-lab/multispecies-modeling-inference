# code for simulating co-occurrence of two interacting species following Araujo &
Rozenfeld (2014)
...
n.A.target = floor(rhoA * L^2) # expected number of ind A in landscape given
prevalence rhoA
...
aggregate(land.r,agg.y,function(x,...){ifelse(4%in%x,4,ifelse(2%in%x&3%in%x,4,max(x)
))})
}
agg.land.true = function(agg.y) {
land.r = raster(land,xmn=0,xmx=L,ymn=0,ymx=L)
aggregate(land.r,agg.y,function(x,...){ifelse(4%in%x,4,max(x))})
}
...
if (is.null(agg)) return(list(rhoAB=rhoAB,rhoAB.null=rhoAB.null,coocc=
cooccAB,land=land)) else
return(list(rhoAB=rhoAB,rhoAB.null=rhoAB.null,coocc= cooccAB,land=land,
land.agg=lapply(agg.l,function(x){data.frame(coordinates(x),cells=values(x))}),land.agg.
true=lapply(agg.l.true,function(x){data.frame(coordinates(x),cells=values(x))}))) } else
if (output=='cooccAB') return(cooccAB) else
if (output=='cooccAB.null') return(list(coocc=cooccAB,null= rhoAB/rhoAB.null))
}
